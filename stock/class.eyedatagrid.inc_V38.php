...
//        }
	...
	*/
	private function buildFooter($shown, $first = 0, $last = 0)
	{
		...
		echo '<tfoot><tr class="tbl-footer"><td class="tbl-nav" colspan="' . $this->column_count . '"><table width="100%" class="tbl-footer"><tr><td width="33%" class="tbl-found">Encontrados <em>' . $this->row_count . '</em> registros';
      ...
      for ($i = $startpage; $i <= $endpage; $i++)
			...
			if ($this->page < $pages)
				echo '<a href="javascript:;" onclick="tblSetPage(' . ($this->page + 1) . ')"><img src="' . $this->image_path . 'arrow_right.gif" class="tbl-arrows" alt="&gt;" title="Pagina Siguiente"></a><a href="javascript:;" onclick="tblSetPage(' . $pages . ')"><img src="' . $this->image_path . 'arrow_last.gif" class="tbl-arrows" alt="&gt;&gt;" title="Ultima Pagina"></a>';
			else
...
//                    echo "Filtro: ".$i." - ".$column." - ".$value."<br>";
                    if ($this->filter['Value']!=''){
            			...
            			} else if ($this->filter['Column'] == "cantidad"){/*Neumaticos*/
                            $filter_query .= "(" . "(select ifnull(sum(cast(m.cantidad as signed)*(case m.tim_id when 1 then 1 else (-1) end)),0)
                                                        from movimientos_stock m
                                                        where m.estado=0
                                                        and m.pro_id = p.pro_id
                                                        and m.suc_id = su.suc_id)" . " LIKE '" . $filter_value . "')";                                			 
