...
$consulta= mysql_query(' select cli_id , cli_nombre , cli_apellido , cli_razon_social , cli_calle , cli_numero
    ...
    , cli_fax , cli_email , cli_cuit , cli_observaciones,cli_estado, cli_tipo_documento, cli_numero_documento from clientes where cli_id = '.$cli_id,$link);
...
function baja_cli(){
    ...
    if ($result1>0 and $result2>0 and $result3>0){
        $sql="update clientes set cli_estado = '1' where cli_id = '$this->cli_id'";
        $result=mysql_query($sql,$link);
        if ($result>0){
            return 1;
        } else {
...
function getclientesCombo($cli_id=0) {
    $link=Conectarse();
    $html = "";
...
function getclientesComboNulo($cli_id=0) {
    $link=Conectarse();
    $html = "";
    $sql="(select cli_apellido, cli_nombre, cli_email, cli_id
        from clientes
        where cli_estado = 0)
        union
        (select ' ' cli_apellido, ' ' cli_nombre, ' ' cli_email, '0' cli_id
        from dual)
        order by cli_email";
    $consulta= mysql_query($sql, $link);
    $html = "<select name='clientes' id='clientes' class='formFields' >";
    while($row= mysql_fetch_assoc($consulta)) {
        if ($cli_id==$row["cli_id"]){
            $html = $html . '<option value='.$row["cli_id"].' selected>'.$row["cli_apellido"].' '.$row["cli_nombre"].' - '.$row["cli_email"].'</option>';
        } else {
            $html = $html . '<option value='.$row["cli_id"].'>'.$row["cli_apellido"].' '.$row["cli_nombre"].' - '.$row["cli_email"].'</option>';
        }
    }
    $html = $html . '</select>';
    return $html;
...
function getclientesCombo_OT($cli_razon_social='',$cli_apellido='',$cli_nombre='',$cli_id='0') {
    $link=Conectarse();
    $html = "";
    $sql="select c.cli_apellido, c.cli_nombre, c.cli_email, c.cli_id
        from clientes c
