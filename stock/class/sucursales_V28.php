...
function sucursales($suc_id=0) {
    if ($suc_id!=0) {
        $link=Conectarse();
        $consulta= mysql_query(' select suc_id , suc_descripcion , suc_calle , suc_numero , suc_piso , suc_departamento , suc_codigo_postal , suc_ciudad , suc_provincia , suc_telefono1 , suc_telefono2 , suc_rubro , suc_numero_sucursal , suc_ultima_factura , suc_ultima_nc , suc_ultima_nd , suc_ultimo_recibo , suc_estado, suc_mail from sucursales where suc_id = '.$suc_id,$link);
...
function baja_suc(){
    ...
    if ($result1>0 and $result2>0 and $result3>0){
        $sql="update sucursales set suc_estado = '1' where suc_id = '$this->suc_id'";
        $result=mysql_query($sql,$link);
        if ($result>0){
            return 1;
        } else {
