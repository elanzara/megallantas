...
function tipo_rango($tr_id=0) {
    if ($tr_id!=0) {
        $link=Conectarse();
        $consulta= mysql_query(' select tr_id , tr_descripcion , tr_velocidad_desde , tr_velocidad_hasta , tr_estado from tipo_rango where tr_id = '.$tr_id,$link);
...
function baja_tr(){
    ...
    if ($result1>0){
        $sql="update tipo_rango set tr_estado = '1' where tr_id = '".$this->tr_id."'";
        $result=mysql_query($sql,$link);
        if ($result>0){
            return 1;
        } else {
