...
if ($mod_id!=0) {
$link=Conectarse();
$consulta= mysql_query(' select mod_id , mar_id , mod_descripcion , mod_estado from modelos where mod_id = '.$mod_id,$link);
...
function baja_mod(){
    ...
    if ($result2>0 and $result3>0 and $result4>0){
        $sql="update modelos set mod_estado = '1' where mod_id = '$this->mod_id'";
        $result=mysql_query($sql,$link);
        if ($result>0){
            return 1;
        } else {
