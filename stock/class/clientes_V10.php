...
{
$link=Conectarse();
$sql="select CASE ifnull(cli_apellido,'') WHEN '' THEN 'z' ELSE cli_apellido END
            , cli_apellido, CASE ifnull(cli_nombre,'') WHEN '' THEN 'z' ELSE cli_nombre END
            , cli_nombre, cli_razon_social, cli_email, cli_id, cli_estado
            , cli_calle, cli_numero, cli_piso, cli_departamento, cli_codigo_postal, cli_ciudad
            , cli_provincia, cli_telefono1, cli_telefono2, cli_fax, cli_cuit, cli_observaciones
            , cli_tipo_documento, cli_numero_documento, usu_id
    from clientes where cli_estado='0'";
if ($texto != "") {
    $sql = $sql ." and (upper(cli_nombre) like (upper('%" . $texto ."%')) "
                ." or upper(cli_apellido) like (upper('%" . $texto ."%')) "
                ." or upper(cli_razon_social) like (upper('%" . $texto ."%')) "
                ." or upper(cli_email) like (upper('%" . $texto ."%'))) ";
}
$sql.=" order by 1, 3, 5";
return $sql;
}
...
function getclientesComboNulo($cli_id=0) {
    $link=Conectarse();
    $html = "";
    $sql="(select cli_nombre, cli_apellido, cli_razon_social, cli_id
        from clientes
        where cli_estado = 0)
        union
        (select  ' ' cli_nombre,' ' cli_apellido, ' ' cli_razon_social, '0' cli_id
        from dual)
        order by cli_nombre, cli_apellido, cli_razon_social";
    $consulta= mysql_query($sql, $link);
    $html = "<select name='clientes' id='clientes' class='formFields' >";
    while($row= mysql_fetch_assoc($consulta)) {
        if ($cli_id==$row["cli_id"]){
            $html = $html . '<option value='.$row["cli_id"].' selected>'.$row["cli_nombre"].' '.$row["cli_apellido"].' - '.$row["cli_razon_social"].'</option>';
        } else {
            $html = $html . '<option value='.$row["cli_id"].'>'.$row["cli_nombre"].' '.$row["cli_apellido"].' - '.$row["cli_razon_social"].'</option>';
        }
    }
    $html = $html . '</select>';
    return $html;
...
function getclientesCombo_OT($cli_razon_social='',$cli_apellido='',$cli_nombre='',$cli_id='0') {
    $link=Conectarse();
    $html = "";
    $sql="select c.cli_nombre, c.cli_apellido, c.cli_razon_social, c.cli_id
        from clientes c
    ...
    $sql.=" order by cli_nombre, cli_apellido, cli_razon_social";
    $consulta= mysql_query($sql, $link);
    $html = "<select name='clientes_ot' id='clientes_ot' class='formFields' >";
    //if ($consulta){
      while($row= mysql_fetch_assoc($consulta)) {
        if ($cli_id==$row["cli_id"]){
            $html = $html . '<option value='.$row["cli_id"].' selected>'.$row["cli_nombre"].' '.$row["cli_apellido"].' - '.$row["cli_razon_social"].'</option>';
