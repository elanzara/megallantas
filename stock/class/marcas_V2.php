...
function baja_mar(){
    ...
    if ($result1>0 and $result2>0 and $result3>0 and $result4>0){
        $sql="update marcas set mar_estado = '1' where mar_id = '$this->mar_id'";
        $result=mysql_query($sql,$link);
        if ($result>0){
            return 1;
        } else {
...
//    return $html;
//}
function getmarcasxTipIdComboNeumatico($tip_id=0,$mar_id=0) {
    $link=Conectarse();
    $html = "";
    $sql="select distinct m.mar_descripcion, m.mar_id
        from marcas m, marcas_tipos_prod t
...
function getmarcas_llantaCombo($mar_id=0) {
    $link=Conectarse();
    $html = "";
    $sql="select distinct m.mar_descripcion, m.mar_id
        from marcas m, marcas_tipos_prod t
