...
$consulta= mysql_query(' select pro_id , mod_id , mar_id , dis_id , prv_id , tip_id , pro_med_diametro , pro_med_ancho
    ...
    , pro_tipo_llanta, pro_material, pro_terminaciones, pro_controla_stock, pro_anio, tr_id, pro_foto, pro_terminacion, pro_clasificacion    
...
function getproductosCombo_MS($tip_id=0,$mar_id=0,$mod_id=0,$pro_med_ancho='',$pro_med_diametro=''
        ,$pro_distribucion='',$tr_id=0,$pro_nueva='',$pro_med_alto='',$pro_descripcion='',$tipo='I',$suc_id='',$pro_id='') {
    $link=Conectarse();
    $html = "";
    $sql_select = "";
    $sql_enc = "";
    ...
    $sql_select="select substr(concat(ifNull(p.pro_descripcion,' ')";
    ...
    $sql="),1,60) pro_descripcion, pro_id
        from productos p
    ...
    if ($pro_med_ancho!='') {
        ...
        if ($tip_id == 2 || $tip_id == 4 || $tip_id == 9){
            $sql_enc.=",' Ancho:',ifNull(p.pro_med_ancho,' ')";
        }
    }
    if ($pro_med_diametro!='') {
        ...
        if ($tip_id == 2 || $tip_id == 4 || $tip_id == 9){
            $sql_enc.=",' Diám:',ifNull(p.pro_med_diametro,' ')";
        }
    }
    if ($pro_distribucion!='') {
        ...
        if ($tip_id == 2 || $tip_id == 4 || $tip_id == 9){
            $sql_enc.=",' Distr:',ifNull(p.pro_distribucion,' ')";
        }
    }
    if ($tr_id!=0) {
        ...
        if ($tip_id == 2 || $tip_id == 4 || $tip_id == 9){
            $sql_enc.=",' Ran:',ifNull(r.tr_descripcion,' ')";
        }
    }
    if ($pro_nueva!='') {
    ...
    if ($pro_med_alto!='') {
        ...
        if ($tip_id == 2 || $tip_id == 4 || $tip_id == 9){
            $sql_enc.=",' Alto:',ifNull(p.pro_med_alto,' ')";
        }
    }
    if ($pro_descripcion!='') {
