...
if ($_SERVER["REQUEST_METHOD"] == "POST" ) {
  ...
  if($_POST["tipo"] == "T"){
    ...
    if ($_POST["sucursales"] != 0 and $_POST["sucursales_des"] != 0 and $_POST["sucursales"] != "" and $_POST["sucursales_des"] != ""){
...
<div id="page"> 
 ...
 <div id="central">
   ...
   <?php
//    echo "SUCURSAL DESTINO:".$_SERVER["suc_des_id"];
   } ?>
     <!--Start FORM -->
     <form ACTION="alta_movimientos_stock.php?tipo=<?php print $tipo.'&tip_id='.$tip_id;?>" METHOD="POST" enctype="multipart/form-data">
        <table align="center" cellpadding="0" cellspacing="1" class="form">
          ...
          <tr>
            ...
            <td class="formFields">
                <input name="pro_id" id="pro_id" type="text" class="campos" size="10" value="<?php print $pro_id_seleccion;?>" />
            </td>
            <td>
                <input type="button" id="busca_producto" name="busca_producto" class="boton" value="Buscar" onclick='seleccionar(<?php echo $_SESSION["suc_id"].",\"".$tipo."\"";?>);' />
            </td>
            <input type="hidden" id="tip_id" name="tip_id" value="<?php echo $tip_id;?>" />
          </tr>
          <?php
//echo"tip_id:".$tip_id." mar_id:".$mar_id." mod_id:".$mod_id;
//          if ($tip_id!='') {
          ?>
<!--              <tr>
...
//   ." dist:".$pro_distribucion." tr:".$tr_id." nueva:".$pro_nueva." alto:".$pro_med_alto;
                    //if (isset($tip_id)) {
                     if ($tip_id!="" and $tip_id!="0") {
                        ...
                        include_once 'class/productos.php';
                        $pro1 = new productos();
                        $res1 = $pro1->getproductosCombo_MS('','','','','','','','','','',$tipo,$_SESSION["suc_id"]
                                ,$pro_id_seleccion);
                        print $res1;
                     } else {
            ...
            <td class="formFields">
                <input name="cantidad" id="cantidad" type="text" class="campos" size="15" />
            </td>
...
<script type="text/javascript">
  function armaFiltro(idSelectOrigen,tipo)
  {
    ...
    if(opcionSeleccionada!=0)
    {
        ...
        location.href = 'alta_movimientos_stock.php?tipo='+tipo+'&tip_id='+opcionSeleccionada+'&fecha='+fecha+'&suc_id='+suc_id+'&suc_des_id='+suc_des_id+'&remito='+remito;
    }
  }
</script>
...
function seleccionar(suc_id, tipo, suc_des_id){
  ...
  if(tipo!='T' || (tipo=='T' && mje_error!='S')){
      if(posicion!=='' && posicion!=0){
        if (posicion==2 || posicion==9){
            window.open('seleccion_producto.php?tip_id='+posicion+'&suc_id='+suc_id+'&tipo='+tipo+'&suc_des_id='+suc_des_id + '&remito=' + remito,'Selecciona producto','scrollbars=No,status=yes,width=1200,height=400,left=200,top=150 1');
        } else if(posicion==4){
            window.open('seleccion_producto_neum.php?tip_id='+posicion+'&suc_id='+suc_id+'&tipo='+tipo+'&suc_des_id='+suc_des_id + '&remito=' + remito,'Selecciona producto','scrollbars=No,status=yes,width=1150,height=400,left=200,top=150 1');
        } else {
            window.open('seleccion_producto_gral.php?tip_id='+posicion+'&suc_id='+suc_id+'&tipo='+tipo+'&suc_des_id='+suc_des_id + '&remito=' + remito,'Selecciona producto','scrollbars=No,status=yes,width=1000,height=400,left=200,top=150 1');
        }
        //window.open('seleccion_producto.php?tip_id='+posicion,'Selecciona producto','scrollbars=No,status=yes,width=1000,height=400,left=200,top=150 1');
          ...
          location.href = 'alta_movimientos_stock.php?tip_id='+posicion+'&suc_id='+suc_id+'&tipo='+tipo
              +'&suc_des_id='+suc_des_id + '&remito=' + remito + '&pro_id_seleccion=' + pro_id;
