					...
					<tr>
						<td style="border-style: none; border-width: medium">
            				<table border="0" width="465" id="table76" cellspacing="0" cellpadding="0" height="40" background="../images/neumaticos/titulos.jpg">
            					<tr>
            						<td width="465">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            						<i class="ofertat">Nuestros Productos </i></td>
            					</tr>
            				</table>
...
<?php
    $gru = new grupos();
    $grupos = $gru->getgrupos($grp_id);
    while ($row = mysql_fetch_assoc($grupos)) {
        echo '<p><i><b>';
		echo '<font face="Arial" color="#FFFFFF" size="4">';
		echo '<a href="productos.php?GRP_ID="'.$row["GRP_ID"].'>';
		echo '<span style="text-decoration: none">';
		echo '<font color="#FFFFFF">'.$row["GRP_DESCRIPCION"].'</font></span></a>:</font></b></i></p>';
        ...
        while ($row2 = mysql_fetch_assoc($categorias)) {
        	...
        	echo '<a href="productos_detalle.php?cat_id='.$row2["CAT_ID"].'">';
        	echo '<img src="../admin/imagenes_categoria/'.$row2["CAT_FOTO"].'" width="90" height="90" border="0" alt="'.$row2["CAT_DESCRIPCION"].'"></a></td>';
        	if ($cont == 3){
                echo '</tr>';
                $cont = 0;
            }
